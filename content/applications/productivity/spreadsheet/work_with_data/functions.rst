...
.. tip::
   Use `F2` to see the formula behind an active cell, and, if desired, edit the formula directly
   in the cell.
...
Odoo Spreadsheet's **Irregularity map** analyzes formulas for patterns and highlight anomalies
visually: areas that contain similar formulas share the same color, while anomalies are shown in a
different color.
...
.. example::
   ...
   The irregularity map identifies the pattern in the formulas in column :guilabel:`D`, and uses the
   same color for cells that match the pattern. The following anomalies are highlighted separately:

   - **Cell D4**: This cell features a static value instead of the formula.
   - **Cell D5**: The formula in this cell, while returning the correct value, does not follow the
     pattern.
   - **Cells D8 and D9**: These cell feature a referencing error due to the missing absolute
     references (`$`) on `G1` in cell :guilabel:`D8`. While the formula returns the correct value in
     the first cell, dragging it to cell :guilabel:`D9` returns no value as the referenced cell
     :guilabel:`G2` is empty.
