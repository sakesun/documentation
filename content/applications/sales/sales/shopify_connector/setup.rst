...
requires a separate account configuration in Odoo, and orders, products, and inventory are scoped per
connected store.
...
- :guilabel:`Default Warehouse Location`: a stock location must be defined, and must belong to the
  selected company. If not configured, Odoo automatically creates a dedicated Shopify stock location.
  In a multi-location setup, click the :guilabel:`Fetch Locations` button in the account
...
- :guilabel:`Update Inventory`: when enabled, the scheduled action that pushes inventory from Odoo to
  Shopify runs for this account. If you do not want to push inventory from Odoo to Shopify, keep it
  disabled.
