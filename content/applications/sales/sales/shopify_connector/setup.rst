...
Accounts`. Alternatively, from the Odoo home page, start typing `Shopify` to open the
:menuselection:`Sales --> Configuration --> Accounts` menu item. Then, click :guilabel:`New`.
...
:ref:`shopify/fulfillment/delivery-modes` section on the *Shopify deliveries, accounting, and
returns* page.
...
.. important::
   To change any of the *Configuration* tab settings, the Shopify account form must be disconnected.
   If the account is already connected, first disconnect it, change the required settings, and then
   connect the account again.

The following settings are available in the *Configuration* tab of the Shopify account form, and
define how data is imported and processed:

- :guilabel:`Create Products`: When enabled, the **Sales** app automatically creates products when
  importing orders or products from Shopify. If disabled, Odoo automatically selects an existing
  product that has the same internal reference as the SKU of the Shopify product. If no matching
  product is found, the default :guilabel:`E-commerce Sale` product is used.
