...
**Commercial invoice scenarios** can be managed through :menuselection:`Customers --> Invoices` for
outgoing invoices and :menuselection:`Vendors --> Bills` for incoming bills.

Commercial invoices
-------------------

On a **non-posted** invoice, select a scenario from the :guilabel:`Invoice Scenario` dropdown field
and specify its kind in the :guilabel:`is` field. Depending on the kind selected, an extra field is
available: :guilabel:`Exemption Reason`.

Once a Commercial scenario invoice has been confirmed and sent, the :guilabel:`Nilvera Status` is
set to :guilabel:`Waiting`. The customer will then respond by either accepting or rejecting the
invoice on their end.

If rejected, the rejection reason text is written in the chatter.

.. note::
   - The invoicing governmental platform wll automatically accept an invoice if the customer does
     not accept or reject the invoice after 8 days from the date of reception.
   - Accepted invoices have two different statuses:
     #. Accepted: Customer accepted manually
     #. Accepted Automatically: 8 day rule applied and government auto-acknowledged the invoice


For Commercial type Bills:

Once system fetches a commercial invoice, a status bar will appear even on the draft stage

User can manually accept, or reject the invoice
If rejected, user has to write a rejection reason
